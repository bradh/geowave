	...
	public CloseableIterator<SimpleFeature> issueQuery(
		...
		try (CloseableIterator<Index> indexIt = getComponents().getDataStore().getIndices()) {
			while (indexIt.hasNext()) {
				...
				// Inspect for SPATIAL_TEMPORAL type index.
				// Most queries issued from GeoServer,
				// where time is an 'enabled' dimension, provide time
				// constraints.
				// Often they only an upper bound. The statistics were used to
				// clip
				// the bounds prior to this point. However, the range
				// may be still too wide. Ideally, spatial temporal indexing
				// should not be used
				// in these cases.
				// Eventually all this logic should be replaced by a
				// QueryPlanner
				// that takes the full set of constraints and has in-depth
				// knowledge of each indices
				// capabilities.
				if ((jtsBounds == null) || (index.getDimensionalityType().equals(
						DimensionalityType.SPATIAL_TEMPORAL) && timeConstraints.isEmpty())) {
					// full table scan
